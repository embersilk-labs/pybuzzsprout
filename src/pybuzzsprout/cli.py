...
@click.option("--podcast-id", required=True, type=int, help="Podcast ID")
@click.option("--json", "as_json", is_flag=True, help="Output raw JSON")
@click.pass_context
def episodes_list(ctx, podcast_id, as_json):
    """List all episodes for a podcast."""
    try:
        client = _get_client(ctx)
        results = client.episodes.list(podcast_id=podcast_id)
        if as_json:
...
@click.option("--podcast-id", required=True, type=int, help="Podcast ID")
@click.option("--episode-id", required=True, type=int, help="Episode ID")
@click.option("--json", "as_json", is_flag=True, help="Output raw JSON")
@click.pass_context
def episodes_get(ctx, podcast_id, episode_id, as_json):
    """Get a specific episode."""
    try:
        client = _get_client(ctx)
        ep = client.episodes.get(podcast_id=podcast_id, episode_id=episode_id)
        if as_json:
...
@click.option("--season-number", default=None, type=int, help="Season number")
@click.option("--episode-number", default=None, type=int, help="Episode number")
...
@click.option("--json", "as_json", is_flag=True, help="Output raw JSON")
@click.pass_context
def episodes_create(ctx, podcast_id, as_json, **kwargs):
    ...
    try:
        ...
        ep = client.episodes.create(podcast_id=podcast_id, **params)
        if as_json:
            click.echo(json_lib.dumps(ep.model_dump(mode="json"), indent=2))
        else:
...
@click.option("--json", "as_json", is_flag=True, help="Output raw JSON")
@click.pass_context
def episodes_update(ctx, podcast_id, episode_id, as_json, **kwargs):
    """Update an existing episode."""
    try:
        client = _get_client(ctx)
        params = {k: v for k, v in kwargs.items() if v is not None}
        ep = client.episodes.update(podcast_id=podcast_id, episode_id=episode_id, **params)
        if as_json:
            click.echo(json_lib.dumps(ep.model_dump(mode="json"), indent=2))
        else:
