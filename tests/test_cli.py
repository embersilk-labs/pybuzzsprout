...
def test_episodes_list():
    ...
    runner = CliRunner(env={"BUZZSPROUT_API_KEY": "test-key"})
    result = runner.invoke(cli, ["episodes", "list", "--podcast-id", str(PODCAST_ID)])
    assert result.exit_code == 0
...
def test_episodes_list_json():
    ...
    runner = CliRunner(env={"BUZZSPROUT_API_KEY": "test-key"})
    result = runner.invoke(cli, ["episodes", "list", "--podcast-id", str(PODCAST_ID), "--json"])
    assert result.exit_code == 0
    data = json.loads(result.output)
    assert len(data) == 1
...
def test_episodes_get():
    ...
    runner = CliRunner(env={"BUZZSPROUT_API_KEY": "test-key"})
    result = runner.invoke(cli, ["episodes", "get", "--podcast-id", str(PODCAST_ID), "--episode-id", "788881"])
    assert result.exit_code == 0
    assert "Too small or too big?" in result.output
